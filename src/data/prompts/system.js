...
7. **Selector**: Generate a CSS selector based on the element's attributes from the [PAGE INTERACTION ELEMENTS] list. Use the most specific attributes available (id, data-*, aria-label, class, etc.) to create a unique selector. Priority: id (#id) > data attributes ([data-attr="value"]) > classes (.class1.class2) > tag name (button, a). The selector must be valid for document.querySelector().`;
