...
{
  "steps": [
    { "text": "Next step instruction (e.g., Click the 'Get Started' button at the top)", "elementIndex": 0 }
  ]
}
...
7. **Element Selection**: You MUST select the most appropriate single element from the [PAGE INTERACTION ELEMENTS] list that matches the step text.
   - Look at the tag and text fields of each element to find the best match.
   - Return the index (0-based) of the matching element in the elementIndex field.
   - **CRITICAL**: You MUST find a matching element and return its index. Only return null for elementIndex if the step text explicitly indicates a non-interactive action (e.g., "read this page", "wait for loading"). For all clickable, input, or interactive elements, you MUST return an elementIndex.
   - Example: If step text is "Click the Build button" and element at index 5 has tag="button" and text="Build", return: { "text": "Click the Build button", "elementIndex": 5 }`;
